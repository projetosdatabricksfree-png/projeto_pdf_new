...
# Unificando o sistema: Adicionando o caminho do projeto para importar o orquestrador oficial
...
try:
    ...
    print(f"❌ Erro ao importar o Orquestrador Oficial: {e}")
    sys.exit(1)
...
def generate_report():
    ...
    print(f"🚀 Iniciando Auditoria Full (MAS Architecture): {len(pdf_files)} patches.")
    master_report = ""
    ...
    for i, path in enumerate(pdf_files):
        ...
        print(f"[{i+1}/{len(pdf_files)}] Processando com Colmeia: {fname}")
        
        # EXECUÇÃO DO SISTEMA REAL
        # O run_all_gwg_checks já roda os 9 agentes em paralelo internamente
        try:
            start_time = time.time()
            engine_result = run_all_gwg_checks(path, profile=FULL_PROFILE, job_id=job_id)
            duration_ms = int((time.time() - start_time) * 1000)
            
            # Extraindo dados normalizados dos agentes
            ...
            # Montagem do Template Elite
            rel = f"# P R E F L I G H T   I N S P E C T O R  ·  R E L A T Ó R I O   D E   V A L I D A Ç Ã O\n"
            rel += f"**{cat}** ({FULL_PROFILE['name']})\n"
            rel += f"Job: `{job_id}`  ·  {time.strftime('%d/%m/%Y')}  ·  Agentes Especialistas: 9 em Paralelo  ·  {duration_ms}ms\n\n"
            ...
            if erros:
                ...
                for e in erros:
                    rel += f"- **{e['label']}** (Cód: {e['codigo']}): {get_action_recommendation(e['label'], 'ERRO')} (Recurso: Pág {e['page']})\n"
                rel += '\n'
    ...
    output_path = '/home/diego/Desktop/PROJETOS/Projeto_grafica/docs/SPRINT_QA/RELATORIO_AUDITORIA_AGENTES_FINAL.md'
    with open(output_path, 'w') as f:
        f.write(master_report)
    print(f"✅ Auditoria Final da Colmeia concluída: {output_path}")
