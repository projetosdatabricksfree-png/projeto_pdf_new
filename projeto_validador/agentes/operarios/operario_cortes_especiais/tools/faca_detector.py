...
def detect_faca_layer(file_path: str) -> dict:
    ...
    try:
        ...
        if "GTS_ProcSteps" in xmp or "ProcessingSteps" in xmp:
            return {"status": "OK", "faca_found": True, "norma": "ISO 19593-1", "detalhe": "Metadados de Processing Steps encontrados"}
        ...
        for xref in range(1, min(doc.xref_length(), 1000)):
            ...
            if "/GTS_ProcSteps" in obj or "/GTS_Metadata" in obj:
                return {"status": "OK", "faca_found": True, "norma": "ISO 19593-1", "detalhe": f"Entry GTS_ProcSteps encontrada no xref {xref}"}
        ...
        for keyword in FACA_NAMES_VALID:
            ...
            for xref in range(1, min(doc.xref_length(), 500)):
                ...
                if keyword.lower() in obj:
                    return {"status": "OK", "faca_found": True, "keyword": keyword, "detalhe": f"Layer detectada por palavra-chave: {keyword}"}
    ...
    try:
        ...
        if "GTS_ProcSteps" in output:
             return {"status": "OK", "faca_found": True, "norma": "ISO 19593-1"}
    except Exception:
        pass
